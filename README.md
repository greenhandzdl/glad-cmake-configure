...
跨平台（Windows / macOS / Linux）的 **现代 OpenGL 图形引擎 / PBR 渲染器**，源自一个用 **GLFW** 建窗、**GLAD** 加载函数的 CMake 起步模板，现已成长为一套分层的 `gfx` 引擎子系统。演示按功能拆分：`src/main.cpp` 只留**最裸的 hello-triangle**（自定义 `RenderPass` + 内联 GLSL 直渲窗口，不碰任何引擎子系统）作为“最小实现”基线，其余一个功能一个入门 demo，集中在 `src/demo/{feature}/main.cpp`（含成品演示 `pbr_showcase`、可玩体素世界 `voxel_terrain`）。依赖通过 CMake 自动探测，内置 `build/run/clean` 脚本与 GitHub Actions 手动发布流水线。
...
.
├── src/
│   ├── main.cpp              # 最裸 hello-triangle：自定义 RenderPass + 内联 GLSL 直渲窗口（最小实现基线，目标 GLFW_Template）
│   ├── demo/                 # 一个功能一个入门 demo；每个含 main.cpp 的子目录被 add_subdirectory 自动收为一个可执行
│   │   ├── CMakeLists.txt    #   add_gfx_demo(<name>) + GLOB 遍历含 main.cpp 的子目录（新增 demo 免改 CMake）
│   │   ├── demo_app.h        #   共享脚手架（非模块）：demo::Run 拥有窗口/渲染线程/析构顺序，demo::Ctx::Loop 拥有帧循环
│   │   ├── demo_cli.h        #   共享命令行开关（非模块）：--quit-after/--on/--off/--help + 数值/字符串选项
...
| **Assimp** | git 子模块（可选） | 由 `GFX_ENABLE_ASSIMP`（默认 `ON`）控制：`ON` 时 `add_subdirectory` 源码内置构建（导入器 only）、链接 `assimp::assimp`；`OFF` 时整个 assimp 依赖不构建、`ModelLoader::Load` 降级为“模型导入不可用”，体素/程序化几何/PBR 不受影响（`model_loading` demo 在 OFF 下会优雅提示不可用而非崩溃） |
...
所有 demo（`src/demo/{feature}/`，含成品 `pbr_showcase` / `voxel_terrain`）都接受同一组共享开关（`src/demo/demo_cli.h`，`--help` 逐项打印该可执行真正认识的功能名），用途是脚本化地
...
`src/main.cpp` 打开一个 800×600 窗口，只画一个三角形：一个自定义 `RenderPass` 在 `Execute` 里 `glClear` + 内联 GLSL 编译的着色器 + `glDrawArrays(3)`，逐帧只填 `fbWidth/fbHeight` 的空 `RenderFrame`。它不建 PBR 管线、不申请 LightBuffer/Scene/后处理——用来证明"只 `import gfx;`、什么都不申请也能出图"，是理解引擎最小使用面的基线。
...
| `model_loading` | AssetManager 异步 + GFX_ENABLE_ASSIMP=OFF 优雅降级 | `./scripts/run.sh model_loading` |
