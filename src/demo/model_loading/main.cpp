 ...
 * hands back a shared_ptr<Model> of already-uploaded gfx::Mesh values. That split
 ...
 * option GFX_ENABLE_ASSIMP. When the engine is built with it OFF, ModelLoader::Load
 * returns std::unexpected("gfx built without Assimp: model import unavailable");
...
class StatusHudPass : public gfx::RenderPass {
public:
    ...
    void Execute(gfx::RenderFrame& f) override {
        ...
        const float w = gfx::TextRenderer::Measure(font_, status_, 20.0f) + 24.0f;
        sprite_.Draw(white_, 14.0f, 14.0f, w, 38.0f, 0, 0, 1, 1, glm::vec4(0, 0, 0, 0.4f));
        gfx::TextRenderer::Draw(sprite_, font_, status_, 26.0f, 22.0f, 20.0f,
...
struct Item { gfx::PbrMaterial material; };
...
int main(int argc, char** argv) {
    ...
    return demo::Run(flags, "gfx demo - model_loading (async AssetManager + graceful OFF)",
                     [&](demo::Ctx& ctx) -> int {
        View view;
        ...
        gfx::Camera camera; camera.SetPerspective(45.0f, 1.0f, 0.1f, 500.0f);
        ...
        } else if (auto probe = gfx::ModelLoader::Load(modelPath); !probe) {
        ...
        std::shared_ptr<gfx::Model> heldModel;   // keep meshes alive once uploaded
            ...
            if (requested && !heldModel) {
                ...
                if (auto m = assets.GetModel("m")) {
                    ...
                    for (const auto& mesh : m->meshes) {
                        ...
                        gfx::SceneNode& n = scene.CreateRoot();
