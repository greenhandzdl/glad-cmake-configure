...
class HudPass : public gfx::RenderPass {
public:
    ...
    void Execute(gfx::RenderFrame& f) override {
        ...
        const char* title = "gfx demo - text_hud";
        char stats[96];
        std::snprintf(stats, sizeof(stats), "%.0f fps   t=%.1fs   fb %dx%d",
                      f.smoothedFps, info_.time, f.fbWidth, f.fbHeight);
        const float panelW = std::max(gfx::TextRenderer::Measure(font_, title, 26.0f),
                                      gfx::TextRenderer::Measure(font_, stats, 20.0f)) + 24.0f;
        sprite_.Draw(white_, 16.0f, 16.0f, panelW, 128.0f,
                     0.0f, 0.0f, 1.0f, 1.0f, glm::vec4(0.0f, 0.0f, 0.0f, 0.4f));

        gfx::TextRenderer::Draw(sprite_, font_, title, 28.0f, 26.0f, 26.0f, glm::vec4(1.0f));
        gfx::TextRenderer::Draw(sprite_, font_, stats, 28.0f, 60.0f, 20.0f,
        ...
        gfx::TextRenderer::Draw(sprite_, font_, body, 28.0f, 86.0f, 18.0f,
        ...
        const float sw = gfx::TextRenderer::Measure(font_, colors[hue], 22.0f);
        gfx::TextRenderer::Draw(sprite_, font_, colors[hue],
...
int main(int argc, char** argv) {
    ...
    return demo::Run(flags, "gfx demo - text_hud (Font + SpriteBatch + TextRenderer)",
                     [&](demo::Ctx& ctx) -> int {
