#ifndef GLDX_SHADER_IBLSHADERS_H
#define GLDX_SHADER_IBLSHADERS_H

/**
 * @file IblShaders.h
 * @brief Embedded GLSL (410 core) for image-based-lighting precomputation.
 *
 * All passes render a fullscreen quad into one cube face at a time. The face
 * orientation is supplied per-draw as three orthonormal basis uniforms
 * (uRight/uUp/uFacing); the sample direction is derived from the quad uv. This
 * avoids any view/projection matrices for the environment passes.
 *
 *   kIblVertex      -> emits uv in [0,1] and a world-space direction.
 *   kSkyGenFragment -> procedural HDR sky (gradient + sun disc), source cube.
 *   kIrradiance     -> cosine-weighted hemisphere convolution of the sky.
 *   kPrefilter      -> GGX importance-sampled specular at a given roughness.
 *   kBrdf           -> Karis split-sum BRDF integration LUT (2D).
 */

namespace gldx::shaders {

inline const char* kIblVertex = R"GLSL(#version 410 core
out vec2 vUV;
...
// Generates one HDR cube face from a procedural sky.
inline const char* kSkyGenFragment = R"GLSL(#version 410 core
in vec2 vUV;
out vec4 FragColor;
uniform vec3 uRight;
uniform vec3 uUp;
...
inline const char* kIrradianceFragment = R"GLSL(#version 410 core
in vec2 vUV;
out vec4 FragColor;
...
// GGX importance-sampled specular prefilter for one roughness (mip) level.
inline const char* kPrefilterFragment = R"GLSL(#version 410 core
in vec2 vUV;
out vec4 FragColor;
uniform samplerCube uEnv;
uniform vec3 uRight;
...
// Karis analytic BRDF integration LUT (environment BRDF).
inline const char* kBrdfFragment = R"GLSL(#version 410 core
in vec2 vUV;
out vec4 FragColor;
...
#endif // GLDX_SHADER_IBLSHADERS_H
