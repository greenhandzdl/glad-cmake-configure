...
/**
 * @file gmf.hpp
 * @brief Shared GLOBAL MODULE FRAGMENT includes for the gldx named module.
 *
 * Every translation unit of module gldx (the primary interface gldx.cppm and each
 * `module gldx;` implementation unit) textually includes THIS header inside its
 * `module;` global module fragment, before the `module gldx;` / `export module
 * gldx;` directive. Doing so attaches GLAD, GLM and the common standard-library
 * entities to the *global module* rather than to module gldx — which is exactly
