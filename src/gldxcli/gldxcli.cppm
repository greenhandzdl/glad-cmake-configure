// ============================================================================
// gldxcli - C++20 named module (primary interface unit)
//
// The command-line switch parser shared by the src/demo/{feature} executables
// and the GLFW_Template main. Formerly the header-only src/demo/demo_cli.h; it
// is now a proper application-side module so a demo reaches it with a plain
// `import gldxcli;` instead of a textual include. It deliberately does NOT
// depend on the engine (gldx) or the window library (gldxwin) - it is pure
// standard-library argument parsing.
//
// Its reason for existing is that a feature can be forced on or off from a
// script instead of from the keyboard, which makes the "toggle everything,
// diff the frame" sweep automatable:
//
//   ./output/voxel_terrain --off fog,water --quit-after 11
//   ./output/pbr_showcase  --on debug --quit-after 6
//
// `--quit-after` lets such a run end on its own, and `--help` prints the names
// the executable actually understands, so a typo in a regression script is
// reported rather than silently ignored.
//
// Global module fragment: the standard headers the class is expressed with are
// textually included here FIRST (before `export module gldxcli;`) so they
// attach to the global module - the same convention the engine module follows
// for glad/glm. Exported members then reference global-module std types.
// ============================================================================
...
export module gldxcli;

export namespace gldx::cli {

class Flags {
public:
    // `features` are the names --off/--on accept for this executable,
    // `options` the names that take a value (--auto-break 20, --model path).
